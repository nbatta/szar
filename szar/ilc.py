...
class ILC_simple:
    def __init__(self,clusterCosmology, \
        ...
        for ii in xrange(len(self.evalells)):
            ...
            self.W_ll_tsz[ii,:] = b_0 * 1./np.dot(np.transpose(f_nu_arr),np.dot(N_ll_for_tsz_inv,f_nu_arr)) \
                                  * np.dot(np.transpose(f_nu_tsz),N_ll_for_tsz_inv)
            self.W_ll_cmb[ii,:] = b_0 * 1./np.dot(np.transpose(f_nu_cmb),np.dot(N_ll_for_cmb_inv,f_nu_cmb)) \
