...
class ClusterCosmology(Cosmology):
    ...
    def __init__(self,paramDict=cosmo.defaultCosmology,constDict=cosmo.defaultConstants,lmax=None,
                 clTTFixFile=None,skipCls=False,pickling=False,fill_zero=True,dimensionless=True,verbose=True,skipPower=True,skip_growth=True):
        Cosmology.__init__(self,paramDict,constDict,lmax,clTTFixFile,skipCls,pickling,fill_zero,dimensionless=dimensionless,verbose=verbose,skipPower=skipPower,skip_growth=skip_growth)
    ...
    def growthfunc(self,z):
        ...
        for i in range(len(a)):
            ...
            integ[i] = np.trapz(integrand, dx=np.diff(aa))
    ...
    def fgrowth(self,z):
        ...
        dlog = np.gradient(np.log(dgrowth), np.log(a))
        #dlna = np.gradient(np.log(a), edge_order=2)
        #dlng = np.gradient(np.log(dgrowth), edge_order=2)
        
        ans = dlog #dlng/dlna #/np.gradient(np.log(a))
        return ans
